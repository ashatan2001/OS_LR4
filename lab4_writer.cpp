...
int main() {
    ...
    hMapped = OpenFileMappingA(GENERIC_WRITE, FALSE, mappedName.c_str());
    ...
    logfileName = BASE_PATH + "writing_logs\\" + to_string(GetCurrentProcessId()) + ".txt";
    logfile = fopen(logfileName.c_str(), "w");

    logfileNameExcel = BASE_PATH + "writing_logs_excel\\" + to_string(GetCurrentProcessId()) + ".txt";
    logfileExcel = fopen(logfileNameExcel.c_str(), "w");

    openSemaphores();

    char* data = new char[4]{ 'l', 'a', 'b', '\0' };
    ...
    while (timeGetTime() < finish) {
        ...
        DWORD page = WaitForMultipleObjects(PAGES, writeSemaphores, FALSE, INFINITE);

        fprintf(logfile, "|State: WRITING; Page: %d|\t\t\t|Time: %d|\n",page, timeGetTime());
        fprintf(logfileExcel, "%d %d\n", timeGetTime(), 1);
        ...
        fprintf(logfile, "|State: RELEASED|\t\t\t\t|ime: %d|\n\n", timeGetTime());
        fprintf(logfileExcel, "%d %d\n", timeGetTime(), 2);
    }
